...
class MySisfallData(Dataset):
    ...
    def __init__(self):
        ...
        """
        adl_12_raw_downsampled = GetPthData(pth_data_dir=args.sisfall_dir,
                                     file_name="SisFall_ADL_12.pth").down_sample(args.sisfall_down)
        adl_25_raw_downsampled = GetPthData(pth_data_dir=args.sisfall_dir,
                                     file_name="SisFall_ADL_25.pth").down_sample(args.sisfall_down)
        adl_100_raw_downsampled = GetPthData(pth_data_dir=args.sisfall_dir,
                                      file_name="SisFall_ADL_100.pth").down_sample(args.sisfall_down)
        fall_15_raw_downsampled = GetPthData(pth_data_dir=args.sisfall_dir,
                                      file_name="SisFall_Fall_15.pth").down_sample(args.sisfall_down)

        """
        feature window 以及 label
        """
        fall_15_fea_window = GetFeaWinFromDataset(multi_axis_dataset=fall_15_raw_downsampled).getWindow()
        self.label = torch.ones(fall_15_fea_window.shape[0], dtype=torch.long)

        adl_12_fea_window = GetFeaWinFromDataset(multi_axis_dataset=adl_12_raw_downsampled).getWindow()
        self.label = torch.cat([self.label, torch.zeros(adl_12_fea_window.shape[0], dtype=torch.long)])

        adl_25_fea_window = GetFeaWinFromDataset(multi_axis_dataset=adl_25_raw_downsampled).getWindow()
        self.label = torch.cat([self.label, torch.zeros(adl_25_fea_window.shape[0], dtype=torch.long)])

        adl_100_fea_window = GetFeaWinFromDataset(multi_axis_dataset=adl_100_raw_downsampled).getWindow()
        ...
        self.signals = modalEmbedding(modal_num=args.modal_three, batch_data=self.signals) # a,g,a
        # self.signals = modalEmbeddingAblation(modal_num=args.modal_three, batch_data=self.signals) # a,g,a
